# ADR 0023: The co-pilot proposes; OpenGravel still decides
...
OpenGravel decides), [ADR 0017](0017-federated-route-candidates.md),
...
1. **"Is this actually the right route?"** OpenGravel's scoring is
...
- Read the `TripPlan` OpenGravel produced and comment on it.
...
|---|---|
| Cannot invent a route | `wouldPick` is validated against the candidate ids OpenGravel supplied; an unknown id drops the whole second opinion. |
| Cannot invent a place | Every coordinate — stop, start, finish, waypoint — comes from a tool result OpenGravel resolved through its own geocoder. The model references a `placeId`; it never emits a latitude. |
...
OpenGravel decides; the co-pilot proposes *inputs and explanations* and the
...
minutes?" is asking about facts OpenGravel already computed and already showed
...
arithmetic OpenGravel already did; it never previews a verdict, and unvalidated
...
pinned through OpenGravel's own geocoder.

Maps grounding also returns directions, travel times, and search-along-route.
**OpenGravel consumes none of it.** Route geometry and route choice stay with
...
the least freedom. A nudge is derived from the plan OpenGravel already computed
...
- `GEMINI_MAPS_GROUNDING=0` drops Maps grounding while leaving the co-pilot
  running on OpenGravel's own data. It defaults **on**, because grounded place
  ...
  to run the co-pilot on OpenGravel's own data alone.
...
This is the first place in OpenGravel where a model's output reaches the rider
