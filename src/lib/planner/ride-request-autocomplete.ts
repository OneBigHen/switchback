...
const CURRENT_LOCATION_LANGUAGE = /^(?:me|here|my\s+(?:current\s+)?location|current\s+location|where\s+i\s+am(?:\s+now)?)$/i

function cleanFragment(value: string): string | null {
  const cleaned = value.replace(/[.!?]+$/, "").trim()
  if (CURRENT_LOCATION_LANGUAGE.test(cleaned)) return null
 ...
 * Current-location phrases stay with the planner's location flow instead of
 * creating nonsense place suggestions for "me" or "here".
 */
