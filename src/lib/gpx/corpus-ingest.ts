...
function normalizeSegment(points: GpxStreamPoint[]): {
  ...
  for (const point of points) {
    ...
    if (
      previous &&
      previous[0] === point.coordinate[0] &&
      previous[1] === point.coordinate[1]
    ) {
      dedupedPointCount += 1;
      continue;
    }
...
function elevationMetrics(groups: SourceGroup[]): {
  ascent: number;
  descent: number;
  ...
  for (const group of groups) {
    for (const segment of group.segments) {
      if (
        !hasElevation &&
        segment.points.some((point) => point.elevationMeters !== null)
      ) {
        hasElevation = true;
      }
      // One shared elevation computation with the Recon adapters: same
      // null-resets-previous convention, no forked arithmetic.
  ...
  const timestamps = groups.flatMap((group) =>
    group.segments.flatMap((segment) =>
      segment.points.flatMap((point) =>
        point.timestampMs === null ? [] : [point.timestampMs],
      ),
    ),
  );
  if (timestamps.length < 2) return 0;
  return Number(
    ((Math.max(...timestamps) - Math.min(...timestamps)) / 60_000).toFixed(2),
  );
}
...
function countGaps(groups: SourceGroup[], maxGapMeters: number): number {
  ...
  for (const group of groups) {
    for (const rawSegment of group.segments) {
      const coordinates = rawSegment.points.flatMap((point) =>
        point.coordinate ? [point.coordinate] : [],
      );
      for (let index = 1; index < coordinates.length; index += 1) {
        if (
          haversine(coordinates[index - 1]!, coordinates[index]!) > maxGapMeters
        )
          gapCount += 1;
      }
    }
    for (let index = 1; index < group.segments.length; index += 1) {
      ...
      if (previous && current && haversine(previous, current) > maxGapMeters)
        gapCount += 1;
    }
  }
  return gapCount;
...
function samplePolyline(
  ...
  for (let index = 1; index < points.length; index += 1) {
    cumulative.push(
      cumulative[index - 1]! + haversine(points[index - 1]!, points[index]!),
    );
  }
  ...
  return Array.from({ length: sampleCount }, (_, index) => {
    ...
    if (next < cumulative.length && cumulative[next] === target)
      return points[next]!;
    const left =
      next < 0 ? points.length - 2 : Math.min(next - 1, points.length - 2);
...
export function createGpxGeometryFingerprint(
  input: {
    geometry: Coordinate[];
    segmentStarts: number[];
    distanceMeters: number;
  },
  sampleCount = FINGERPRINT_SAMPLE_COUNT,
): GpxGeometryFingerprint {
  const starts = input.segmentStarts.filter(
    (start) =>
      Number.isInteger(start) && start >= 0 && start < input.geometry.length,
  );
...
export function areGpxFingerprintsNear(
  ...
  if (left.segmentCount === 0 || left.segmentCount !== right.segmentCount)
    return false;
  ...
  if (
    haversine(left.start, right.start) > maxEndpointDistanceMeters ||
    haversine(left.end, right.end) > maxEndpointDistanceMeters
  )
    return false;
  if (left.samples.length !== right.samples.length || left.samples.length === 0)
    return false;
  const maxPointDistanceMeters = options.maxPointDistanceMeters ?? 100;
  return left.samples.every(
    (point, index) =>
      haversine(point, right.samples[index]!) <= maxPointDistanceMeters,
  );
}
...
export function splitGpxDocument(
  ...
  const minimumRideMeters =
    options.minimumRideMeters ?? DEFAULT_MINIMUM_RIDE_METERS;
  ...
  for (const source of sources) {
    for (const segment of source.segments) {
      for (const point of segment.points) {
        if (!point.coordinate) continue;
        if (spacingPrevious)
          spacings.push(haversine(spacingPrevious, point.coordinate));
  ...
  const medianSpacing =
    spacings.toSorted((a, b) => a - b)[Math.floor(spacings.length / 2)] ?? 0;
  const breakMeters = Math.max(
    splitGapMeters,
    medianSpacing * SPARSE_SPACING_MULTIPLE,
  );
  ...
  for (const source of sources) {
    for (const segment of source.segments) {
      for (const point of segment.points) {
        ...
        if (
          previous !== null &&
          haversine(previous, point.coordinate) > breakMeters
        ) {
          closeRun();
          runName = source.name;
        }
  ...
  const rides = runs.filter(
    (candidate) => runDistance(candidate.points) >= minimumRideMeters,
  );
  if (rides.length === 0) {
    ...
    if (runs.length > MAX_RIDES_PER_FILE) {
      throw new Error(
        `The GPX document is a road collection, not a ride: ${runs.length} disconnected pieces`,
      );
    }
  ...
  if (rides.length === 1) {
    ...
    return [
      normalizeGpxDocument(
        documentForRun(document, rides[0]!, document.waypoints),
        options,
      ),
    ];
  }
  if (rides.length > MAX_RIDES_PER_FILE) {
    throw new Error(
      `The GPX document is a road collection, not a ride: ${rides.length} disconnected pieces over ${Math.round(minimumRideMeters / 1609.344)} mi`,
    );
  }
  ...
  return rides.flatMap((ride, index) => {
    try {
      ...
      const part = documentForRun(
        document,
        ride,
        index === 0 ? document.waypoints : [],
      );
      return [
        normalizeGpxDocument(part, {
          ...options,
          id: `${options.id}--t${index + 1}`,
        }),
      ];
    } catch {
      return [];
...
export function normalizeGpxDocument(
  ...
  if (geometry.length < 2)
    throw new Error("The GPX document has no valid route geometry");
  ...
  const distanceMeters = segments.reduce(
    (total, segment) => total + segmentDistance(segment),
    0,
  );
  const name = (
    document.metadataName ||
    document.tracks[0]?.name ||
    document.routes[0]?.name ||
    fallbackName(options.fileName)
  ).slice(0, 160);
  const waypoints = document.waypoints.flatMap((waypoint) =>
    waypoint.coordinate
      ? [
          {
  ...
  const fingerprint = createGpxGeometryFingerprint({
    geometry,
    segmentStarts,
    distanceMeters,
  });
  ...
  return {
    ...
    ascentMeters: elevation.hasElevation
      ? Number(elevation.ascent.toFixed(1))
      : null,
    descentMeters: elevation.hasElevation
      ? Number(elevation.descent.toFixed(1))
      : null,
    dedupedPointCount,
    gapCount,
