/**
 * Shared constants, types and pure helpers for the route atlas.
 *
 * Deliberately free of `node:` imports: route cards and the discovery map
 * recover real geography from this art in the browser, and a value import of a
 * module that touches `node:fs` is enough to break the client build with a
 * chunking error that names no file.
 *
 * `atlas.ts` re-exports all of this and adds the disk reads.
 */
...
export function isAtlasRecord(value: unknown): value is Record<string, unknown> {
...
function isMiniPath(value: unknown): value is AtlasMiniPath {
  if (!isAtlasRecord(value) || typeof value.d !== "string" || value.d.length === 0 || !isBand(value.band)) return false
...
export function parseRouteArt(value: unknown): AtlasRouteArt | null {
  if (!isAtlasRecord(value) || !Array.isArray(value.paths) || value.paths.length === 0) return null
