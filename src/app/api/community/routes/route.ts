...
function invalidateCommunityRoutes(routeId?: string): void {
  try {
    revalidatePath("/routes")
    revalidatePath("/api/community/routes")
    if (routeId) revalidatePath(`/routes/${routeId}`)
  } catch {
    // Dynamic pages remain database-backed; cache invalidation is best-effort.
  }
}
...
export async function handleCommunityRoutesPost(request: Request, store = getCommunityStore()): Promise<Response> {
  ...
  try {
    ...
    return apiErrorResponse("AUTH_REQUIRED", "A verified OpenGravel ID is required to publish a route.", 401, requestId)
  }
  try {
    const body = await readBoundedJsonBody(request, 512 * 1024)
    const draft = parseCommunityRouteDraft(body)
    const created = store.createRoute(identityId, draft)
    invalidateCommunityRoutes(created.routeId)
