...
export async function handleRigContributionPost(
  ...
  try {
    ...
    if (caught instanceof Error && caught.message === "AUTH_REQUIRED") return apiErrorResponse("AUTH_REQUIRED", "A verified OpenGravel ID is required to contribute RIG evidence.", 401, requestId)
