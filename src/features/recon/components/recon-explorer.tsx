...
export default function ReconExplorer({
  ...
  const resolvedSelectedId =
    selectedId ??
    (recordedTracks !== null && recordedTracks.length > 0
      ? recordedTracks[0]!.id
      : null) ??
    ...
    fetchCatalogRoute(resolvedSelectedId, fetcher)
      .then((route) => {
        ...
        setCatalogTracks((previous) =>
          new Map(previous).set(resolvedSelectedId, track),
        );
      })
      .catch(() => {
  ...
  const loading =
    recordedTracks === null || (catalogEntries === null && !catalogUnavailable);
  const hasAnyTrack =
    (recordedTracks?.length ?? 0) > 0 || (catalogEntries?.length ?? 0) > 0;
        ...
        <>
          ...
          {selectedTrack ? (
            <section className={styles.hud} aria-live="polite">
              ...
              {previewError ? (
                <p className={styles.hudMeta}>{previewError}</p>
              ) : null}
            </section>
          ) : null}
        </>
