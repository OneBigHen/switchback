...
export default function ReconTrackMap({ track }: { track: ReconTrack | null }) {
  ...
  useEffect(() => {
    ...
    const onLoad = () => {
      ...
      map.addLayer(trackEndpointLayer("recon-track-start", "start", PAPER, EMBER))
...
function applyTrack(map: maplibregl.Map, track: ReconTrack | null): void {
  ...
  if (!source) return
  ;(source as GeoJSONSource).setData(track ? trackCollection(track) : emptyCollection())
  if (track) fitToTrack(map, track)
}

function fitToTrack(map: maplibregl.Map, track: ReconTrack): void {
  const bounds = new maplibregl.LngLatBounds()
  for (const coordinate of track.geometry.coordinates) bounds.extend(coordinate)
...
function trackLineLayer(): LineLayerSpecification {
  return {
    ...
    paint: {
      ...
      "line-color": ["match", ["get", "playbackKind"], "recorded", EMBER, SIGNAL],
