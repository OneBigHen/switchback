...
function routePath(seed: string): string {
  const hash = hashSeed(seed)
  const x = [5, 19, 33, 48, 63, 78, 95]
  const y = x.map((_, index) => 14 + Math.round(sample(hash, index) * 54))
  return x.map((point, index) => `${index === 0 ? "M" : "L"}${point} ${y[index]}`).join(" ")
}
...
export function RouteGraphic({ seed, variant, label, className }: RouteGraphicProps) {
  ...
  return (
    ...
    >
      ...
      <path className={styles.traceHalo} d={routePath(`${seed}:${variant}`)} />
      <path className={styles.trace} d={routePath(`${seed}:${variant}`)} data-route-trace="true" />
      <circle className={styles.start} cx="5" cy={routePath(`${seed}:${variant}`).match(/^M5 (\d+)/)?.[1] ?? "40"} r="2.6" />
    </svg>
  )
}
