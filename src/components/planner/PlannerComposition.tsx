...
import {
  ...
  type PlannerPresentationBoundary,
  type PlannerPresentationInput,
  type PlannerRouteComparisonProps
...
/**
 * Legacy adapter for the current PlannerShell. New orchestration should build
...
export interface PlannerCompositionBoundaryProps {
  boundary: PlannerPresentationBoundary
}
 ...
 *
 * This component intentionally knows only presentation models + commands. It
...
export function PlannerCompositionBoundary({ boundary }: PlannerCompositionBoundaryProps) {
  ...
  const openDetails = (id: string) => {
    comparison?.onSelect(id)
    if (comparison) setDetails(openRouteDetails(id, comparison.routes))
  }
