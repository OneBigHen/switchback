...
node="$(hostname -s)"
expected_node="${CI_NODE:?set CI_NODE to the Proxmox host name}"
[[ "$node" == "$expected_node" ]] || die "refusing to run on $node; expected $expected_node"

vmid="${CI_VMID:-$(pvesh get /cluster/nextid)}"
name="${CI_HOSTNAME:-switchback-ci}"
storage="${CI_STORAGE:?set CI_STORAGE to the target Proxmox storage}"
...
bridge="${CI_BRIDGE:?set CI_BRIDGE to the target network bridge}"
