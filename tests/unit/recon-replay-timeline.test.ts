...
import { makePlannedRoute, makeRecordedRide, TEST_RIDE_START_MS } from "./fixtures/recon-fixtures"
...
describe("sampleReplay on recorded tracks", () => {
  ...
  it("keeps unknown speed and altitude unknown during interpolation", () => {
    const partial = adaptRecordedRide(
      makeRecordedRide({
        points: [
          { coordinate: [0, 0], recordedAt: isoAt(0), speedMph: null, altitudeMeters: 100 },
          { coordinate: [0, 0.001], recordedAt: isoAt(60_000), speedMph: 20, altitudeMeters: null },
        ],
      })
    )!
  ...
  it("never mutates the track", () => {
    ...
    for (let position = 0; position <= 1; position += 0.05) sampleReplay(track, position)
...
describe("sampleReplay fails safely on impossible tracks", () => {
  function literalTrack(points: ReconTrackPoint[], playbackKind: "recorded" | "preview"): ReconTrack {
    return {
      id: "literal",
      name: "Literal",
      sourceKind: playbackKind === "recorded" ? "recorded-ride" : "catalog-route",
      playbackKind,
  ...
  function pointAt(coordinate: [number, number], recordedAt: number | null): ReconTrackPoint {
    return { coordinate, recordedAt, speedMph: null, altitudeMeters: null, headingDegrees: null, accuracyMeters: null }
  }

  it("rejects decreasing recorded timestamps instead of inventing order", () => {
    const backwards = literalTrack(
      [pointAt([0, 0], 120_000), pointAt([0, 0.001], 60_000), pointAt([0, 0.002], 0)],
      "recorded"
    )
  ...
  it("tolerates recorded stops with equal adjacent timestamps", () => {
    const withStop = adaptRecordedRide(
      makeRecordedRide({
        points: [
          { coordinate: [0, 0], recordedAt: isoAt(0), speedMph: 10, altitudeMeters: 100 },
          { coordinate: [0, 0], recordedAt: isoAt(30_000), speedMph: 0, altitudeMeters: 100 },
          { coordinate: [0, 0.002], recordedAt: isoAt(120_000), speedMph: 30, altitudeMeters: 120 },
        ],
      })
    )!
