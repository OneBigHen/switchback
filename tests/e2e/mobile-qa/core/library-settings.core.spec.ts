...
test("fresh empty rides are honest and reachable on mobile", async ({ page, mobileQa }, testInfo) => {
  ...
  await expect(page.getByText("Import a route file, save a planned route, or save one from Explore routes to start My Rides.")).toBeVisible()
...
test.describe("reload persistence", () => {
  ...
  test("settings preserve a safe local preference and light/dark theme across reload", async ({ page, mobileQa }, testInfo) => {
    ...
    await expect(page.getByRole("textbox", { name: "Rider name" })).toHaveValue("")
    await expectFocusedControlInVisualViewport(page, "Rider name")
    await page.getByRole("textbox", { name: "Rider name" }).fill("Local mobile rider")
    ...
    await expect(page.getByRole("textbox", { name: "Rider name" })).toHaveValue("Local mobile rider")
    await expect(page.getByRole("combobox", { name: "Units" })).toHaveValue("metric")
