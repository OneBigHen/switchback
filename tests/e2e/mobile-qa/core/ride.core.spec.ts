...
test.describe("Level A mobile ride scenarios", () => {
  ...
  test("recording starts, updates with two bounded GPS samples, and stops", async ({ mobileQa }, testInfo) => {
    ...
    const finishRecording = mobileQa.page.getByRole("button", { name: "Finish recording" })
    await tapControlByTouch(mobileQa.page, finishRecording)
...
test.describe("Level A recording recovery", () => {
  ...
  test("interrupted recording reloads into a recoverable paused state", async ({ mobileQa }, testInfo) => {
    ...
    const finishRecoveredRecording = mobileQa.page.getByRole("button", { name: "Finish & save" })
    await tapControlByTouch(mobileQa.page, finishRecoveredRecording)
  })
})
