...
test("recon explorer opens with map, picker sections, no overflow and no console errors", async ({ page }) => {
...
test("a route preview loads from the picker into the HUD", async ({ page, request }) => {
  const { consoleErrors, pageErrors } = collectConsoleErrors(page)
  ...
  await expect
    .poll(
      () =>
        page.evaluate(
          () => window.__reconMapDebug?.getPitch() ?? 0,
        ),
      { timeout: 30_000 },
    )
...
test("the focused ride view deep-links to the same route", async ({ page, request }) => {
  const { consoleErrors, pageErrors } = collectConsoleErrors(page)
